   ...
   e:	83 39 03             	cmpl   $0x3,(%ecx)
{
  ...
  22:	6a 02                	push   $0x2
  24:	e8 17 05 00 00       	call   540 <printf>
    exit();
  29:	e8 a5 03 00 00       	call   3d3 <exit>
  }
  ...
  36:	e8 f8 03 00 00       	call   433 <link>
  3b:	83 c4 10             	add    $0x10,%esp
  3e:	85 c0                	test   %eax,%eax
  40:	78 05                	js     47 <main+0x47>
    printf(2, "link %s %s: failed\n", argv[1], argv[2]);
  exit();
  42:	e8 8c 03 00 00       	call   3d3 <exit>
    printf(2, "link %s %s: failed\n", argv[1], argv[2]);
  47:	ff 73 08             	pushl  0x8(%ebx)
  4a:	ff 73 04             	pushl  0x4(%ebx)
  4d:	68 bb 08 00 00       	push   $0x8bb
  52:	6a 02                	push   $0x2
  54:	e8 e7 04 00 00       	call   540 <printf>
  59:	83 c4 10             	add    $0x10,%esp
  5c:	eb e4                	jmp    42 <main+0x42>
  5e:	66 90                	xchg   %ax,%ax

00000060 <thread_create>:
#include "user.h"
#include "x86.h"
...
thread_create(void (*start_routine)(void *, void *), void *arg1, void *arg2) 
{
  ...
  6b:	68 00 10 00 00       	push   $0x1000
  70:	e8 2b 07 00 00       	call   7a0 <malloc>
  if(stack == 0)
  75:	83 c4 10             	add    $0x10,%esp
  78:	85 c0                	test   %eax,%eax
  7a:	74 32                	je     ae <thread_create+0x4e>
  ...
  7e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  83:	05 00 10 00 00       	add    $0x1000,%eax
  88:	f7 c3 ff 0f 00 00    	test   $0xfff,%ebx
  8e:	0f 45 d8             	cmovne %eax,%ebx
  }

  // Call clone system call to create the thread
  pid = clone(start_routine, arg1, arg2, stack);
  91:	53                   	push   %ebx
  92:	ff 75 10             	pushl  0x10(%ebp)
  95:	ff 75 0c             	pushl  0xc(%ebp)
  98:	ff 75 08             	pushl  0x8(%ebp)
  9b:	e8 d3 03 00 00       	call   473 <clone>
  
  if(pid == 0) {
  a0:	83 c4 10             	add    $0x10,%esp
  a3:	85 c0                	test   %eax,%eax
  a5:	74 02                	je     a9 <thread_create+0x49>
    // Child thread - just return 0
    return 0;
  } else if(pid < 0) {
  a7:	78 0c                	js     b5 <thread_create+0x55>
    return -1;
  }
  ...
  b3:	eb f4                	jmp    a9 <thread_create+0x49>
    free(stack);
  b5:	83 ec 0c             	sub    $0xc,%esp
  b8:	53                   	push   %ebx
  b9:	e8 52 06 00 00       	call   710 <free>
    return -1;
  be:	83 c4 10             	add    $0x10,%esp
  c1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  c6:	eb e1                	jmp    a9 <thread_create+0x49>
  c8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  cf:	90                   	nop

000000d0 <thread_join>:
...
thread_join(void) 
{
  ...
  d7:	83 ec 34             	sub    $0x34,%esp
  void *stack;
  int pid;
  
  // Wait for a thread to exit
  pid = join(&stack);
  da:	8d 45 f4             	lea    -0xc(%ebp),%eax
  dd:	50                   	push   %eax
  de:	e8 98 03 00 00       	call   47b <join>
  
  if(pid > 0) {
  e3:	83 c4 10             	add    $0x10,%esp
  e6:	85 c0                	test   %eax,%eax
  e8:	7f 06                	jg     f0 <thread_join+0x20>
  ...
  f3:	ff 75 f4             	pushl  -0xc(%ebp)
  f6:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  f9:	e8 12 06 00 00       	call   710 <free>
  fe:	8b 45 e4             	mov    -0x1c(%ebp),%eax
...
lock_init(lock_t *lock) 
{
 ...
 11a:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lock->turn = 0;
 120:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
}
 127:	5d                   	pop    %ebp
 128:	c3                   	ret    
 129:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

00000130 <lock_acquire>:
...
xchg(volatile uint *addr, uint newval)
{
 ...
 135:	b9 01 00 00 00       	mov    $0x1,%ecx
 ...
 147:	75 f7                	jne    140 <lock_acquire+0x10>
    ; // Spin
}
 149:	5d                   	pop    %ebp
 14a:	c3                   	ret    
 ...
 49c:	89 d1                	mov    %edx,%ecx
{
 ...
 4c9:	0f b6 92 d8 08 00 00 	movzbl 0x8d8(%edx),%edx
...
printf(int fd, const char *fmt, ...)
{
 ...
 6c4:	eb 1a                	jmp    6e0 <printf+0x1a0>
          s = "(null)";
 6c6:	bb cf 08 00 00       	mov    $0x8cf,%ebx
        while(*s != 0){
...
free(void *ap)
{
 ...
 715:	a1 34 0c 00 00       	mov    0xc34,%eax
{
 ...
 736:	73 10                	jae    748 <free+0x38>
{
 ...
 761:	5b                   	pop    %ebx
  freep = p;
 762:	a3 34 0c 00 00       	mov    %eax,0xc34
}
 767:	5e                   	pop    %esi
 768:	5f                   	pop    %edi
 769:	5d                   	pop    %ebp
 ...
 787:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 78a:	a3 34 0c 00 00       	mov    %eax,0xc34
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 7ad:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 7b0:	8b 3d 34 0c 00 00    	mov    0xc34,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 7f9:	8b 3d 34 0c 00 00    	mov    0xc34,%edi
 ...
 854:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 857:	a3 34 0c 00 00       	mov    %eax,0xc34
}
 ...
 86e:	66 90                	xchg   %ax,%ax
    base.s.ptr = freep = prevp = &base;
 870:	c7 05 34 0c 00 00 38 	movl   $0xc38,0xc34
 877:	0c 00 00 
    base.s.size = 0;
 87a:	bf 38 0c 00 00       	mov    $0xc38,%edi
    base.s.ptr = freep = prevp = &base;
 87f:	c7 05 38 0c 00 00 38 	movl   $0xc38,0xc38
 886:	0c 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 889:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 88b:	c7 05 3c 0c 00 00 00 	movl   $0x0,0xc3c
 892:	00 00 00 
    if(p->s.size >= nunits){
